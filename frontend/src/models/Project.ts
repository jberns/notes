...
export const Note = types
  .model({
    ...
    type: types.optional(
      types.enumeration<NoteType>('NoteType', Object.values(NoteType)),
      NoteType.note,
    ),
...
export const Page = types
  .model({
    ...
    blocks_ref: types.optional(
      types.array(types.safeReference(Block, { acceptsUndefined: false })),
      [],
    ),
  })
  .actions((self) => ({
...
export const Project = types
  ...
  .views((self) => ({
    allTasks() {
      const filteredTasks: IBlock[] = self.pages.reduce(
        (tasks: IBlock[], page) => {
          if (page.blocks_ref) {
      ...
      const filteredTasks: IBlock[] = self.pages.reduce(
        (tasks: IBlock[], page) => {
          if (page.blocks_ref) {
            const onlyTasks = page.blocks_ref.filter((block) => {
              return (
                block.content.type === NoteType.task &&
                block.content.complete === false
              );
            });
...
export const RootStore = types
  .model({
    ...
    copiedNote: types.maybeNull(
      types.safeReference(Note, { acceptsUndefined: false }),
    ),
  })
  .actions((self) => ({
